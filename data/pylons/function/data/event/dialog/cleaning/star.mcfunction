# 清除无效项
$execute store success score #temp NatureCraft.var run data remove storage pylons:main var.input.star[{id:$(id)}]
# 刷新计数器
...
execute unless score #temp NatureCraft.var matches 0 run function pylons:data/event/dialog/cleaning/star with storage pylons:main var.input.bin[0]